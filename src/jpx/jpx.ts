    ...
    parseImageProperties: function JpxImage_parseImageProperties(stream) {
      ...
      while (newByte >= 0) {
        ...
        if (code === 0xff51) {
          ...
          // Read first component precision from the SIZ component info
          // (3 bytes per component: precision|signed, XRsiz, YRsiz)
          var precisionByte = stream.getByte();
          this.bitsPerComponent = (precisionByte & 0x7f) + 1;
          return;
        }
      }
  ...
  function transformComponents(context) {
    ...
    for (var i = 0, ii = context.tiles.length; i < ii; i++) {
      ...
      if (tile.codingStyleDefaultParameters.multipleComponentTransform) {
        ...
        // no multi-component transform — GRIB2 full-precision variant
        for (c = 0; c < componentsCount; c++) {
